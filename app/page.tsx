...
export default function Home() {
  ...
  return (
    <main className="select-none flex flex-col  gap-3 md:gap-8 container  items-center justify-center lg:pt-16">
      <h2 className='font-heading text-center md:text-6xl text-3xl bg-gradient-to-r from-[#e0c24c] via-[#54634c] to-green-800 bg-clip-text leading-relaxed text-transparent'>Welcome to the Yari Token  <br className='hidden md:block' /> Burning Interface </h2>
      <p className='text-muted-foreground font-heading text-xl max-w-3xl text-center'>Unleash your cypherpunk spirit and become a true DeFi degenerate while shaping the evolution of the Yari protocol and stacking those juicy rewards!
      </p>
      <p className='text-foreground font-sans max-w-3xl text-center'>As a Yari hodler, you wield the power to propose and implement radical governance models and unleash the wildest liquidity experiments on the Bitcoin network.
      ...
      <form className='mt-3 rounded-xl flex flex-col justify-center items-center w-full md:w-8/12 lg:w-5/12 border-1  gap-5 ' onSubmit={approveToken}>
